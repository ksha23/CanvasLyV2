...
const getEventsFromGoogle = async (req, res) => {
  ...
  const response = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events?timeMin=${today.toISOString()}&maxResults=30`, {
    headers: {
      Authorization: `Bearer ${req.user.accessToken}`
    }
  });
...
router.get('/events', _requireJwtAuth.default, _refreshAccessToken.default, async (req, res) => {
  let timeZone = '';
  try {
...
router.get('/calendarById/:id', _requireJwtAuth.default, _refreshAccessToken.default, async (req, res) => {
  const {
    id
  } = req.params;
  try {
