...
const Navbar = ({ auth, logOutUser, history }) => {
  ...
  return (
    <nav className="bg-white text-black px-4 pt-4 pb-2 dark:text-white dark:bg-black">
      <div className="sticky top-0 flex items-center justify-between">
        <div className="flex items-center">
          ...
          {!auth.isAuthenticated && <p className="font-semibold ml-2">CanvasLy</p>}
          {auth.isAuthenticated && (
            <>
              <Link
                to="/assignments"
                className={`ml-4 text-black text-s md:text-base dark:text-white ${isActiveLink(
                  '/assignments',
                )}`}
              >
                Assignments
              ...
              <Link
                to={`/${auth.me.username}`}
                className={`ml-4 text-black text-s hidden md:block md:text-base dark:text-white ${isActiveLink(
                  `/${auth.me.username}`,
                )}`}
              >
                Profile
        ...
        <div className="flex items-center space-x-4">
          ...
          {!auth.isAuthenticated && (
            <Link
              className="px-3 py-1 text-white rounded bg-gradient-to-bl from-sky-400 to-indigo-900"
              to="/login"
            >
              Log in
            </Link>
